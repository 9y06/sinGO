...
def resolve_report():
    access_code = input("권한이 필요합니다. 비밀번호를 입력하세요: ")
    if access_code == "teacher_password":
        report_number = int(input("처리할 신고 번호: ")) - 1
        if 0 <= report_number < len(reports):
            reports[report_number]["resolved"] = True
            print(f"불편 사항 {report_number + 1}이 완료 상태로 변경되었습니다.")
        else:
            print("유효한 신고 번호를 입력하세요.")
    else:
        print("권한이 없습니다.")

    # 여기에서 실제 권한 확인 및 처리 로직 추가 가능
